...
export const create = async (name: string, pets: string[], repeat: boolean, ending: boolean, date: Date, endDate: Date | null, frequency: string | null, times: number | null): Promise<Care>  => {
  return (await axios.post<Care>(BASE_URL, { name, pets, repeat, ending, date, endDate, frequency, times })).data
}

export const update = async (name: string, pets: string[], repeat: boolean, ending: boolean, date: Date, endDate: Date | null, frequency: string | null, times: number | null, careId: string): Promise<Care>  => {
